...
│   │   ├── training.py               # [已实现] 训练请求、表单校验、参考/续训产物、进度事件和训练分发
...
│   │   │   ├── training.py           # [已实现] 模型训练 Tk 控制器，负责控件、队列事件、进度和预览刷新
│   │   │   ├── training_options.py   # [已实现] 训练参考模型和继续训练下拉标签映射
...
│   └── test_training_workflow.py     # [已实现] 训练 workflow 请求解析、分发和进度事件测试
│
├── docs/                             # [已实现/待扩展] 项目文档
...
- `[已实现]` GUI 内可训练敌对 AI，并保存为时间戳成果；`latest/` 默认模型由结果管理手动发布。
- `[已实现]` GUI 训练完成后自动重新加载 AI 模型。
- `[已实现]` GUI 内实验进度条与结果保存提示。
- `[已实现]` 训练面板按 `ui -> workflows -> domain` 分层：UI 负责 Tk 控件和队列事件，`workflows.training` 负责请求校验、进度归一化和训练分发，`domain.train` 负责实际算法。
...
- `[已实现]` DQN 稳定性配置与控制器测试。
- `[已实现]` 结果管理服务测试。
- `[已实现]` 训练 workflow 请求解析、Q/DQN 分发和进度事件归一化测试。
